    ...
    protected void onDraw(Canvas canvas) {
        for (int i = 0; i < mGradeNumber; i++) {
            if (mCurrentGradeNumber > i) {
                float bitmapLeft = i * mBitMapFocus.getWidth() + (float) (i * mHorizontalDirectionPadding / (mGradeNumber - 1));
                float bitmapTop = (float) (mVerticalDirectionPadding / 2);
                canvas.drawBitmap(mBitMapFocus,
                        bitmapLeft,
                        bitmapTop, null);
            } else {
                float bitmapLeft = i * mBitMapNormal.getWidth() + (float) (i * mHorizontalDirectionPadding / (mGradeNumber - 1));
                float bitmapTop = (float) (mVerticalDirectionPadding / 2);
                canvas.drawBitmap(mBitMapNormal, bitmapLeft, bitmapTop, null);
            }
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                //event.getRawX()获取屏幕的x位置，event.getX()获取当前控件的x位置
