...
class RatioRelativeLayout : RelativeLayout {
    ...
    private fun initAttr(context: Context, attrs: AttributeSet?) {
        val typedArray =
            context.obtainStyledAttributes(attrs, R.styleable.RatioRelativeLayout)?.also {
                val ratio =
                    it.getString(R.styleable.RatioRelativeLayout_viewGroupDimentsRatio) ?: ""
                try {
                    when {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        super.onMeasure(
            setMeasureSpec(widthMeasureSpec, heightMeasureSpec, 1),
            setMeasureSpec(widthMeasureSpec, heightMeasureSpec, 2)
        )
    }
    ...
    private fun setMeasureSpec(widthMeasureSpec: Int, heightMeasureSpec: Int, type: Int): Int {
        ...
        return if (MeasureSpec.getMode(widthMeasureSpec) == MeasureSpec.EXACTLY) {
            ...
            if (MeasureSpec.getMode(heightMeasureSpec) == MeasureSpec.EXACTLY) {
                ...
                if (type == 1) widthMeasureSpec else MeasureSpec.makeMeasureSpec(
                    (widthSize * mHeightRatio / mWidthRatio).toInt(),
                    MeasureSpec.EXACTLY
                )
            }
        } else if (MeasureSpec.getMode(heightMeasureSpec) == MeasureSpec.EXACTLY) {
            //高=精确
            if (type == 1) MeasureSpec.makeMeasureSpec(
                (heightSize * mWidthRatio / mHeightRatio).toInt(),
                MeasureSpec.EXACTLY
