package com.tzt.customizekt.diy.dashboard;
...
import static com.tzt.customizekt.study.Utils.dp2px;
 ...
 * https://github.com/zhengweichao/view
 */
...
//        关闭硬件加速-View需要用到发光的效果，我们采用 shader 来实现，那么我们需要首先关闭硬件加速。
...
//        旋转画布 （坐标系）
        ...
        for (int i = 0; i < clockPointNum + 1; i++) {
            ...
            //2、每次旋转240/100
            canvas.rotate((360 - openAngle) / clockPointNum);//240/100
        }
        // （180-120）/2 + （360-120）/100
...
//        canvas.rotate(360 - 150 - ((360 - openAngle) / clockPointNum) * i);        //坐标系总旋转角度为360度
        canvas.rotate(((360 - openAngle) / clockPointNum) * i);        //坐标系总旋转角度为360度
        Log.e("shi", "------>rotate=" + ((360 - openAngle) / clockPointNum) * i);
