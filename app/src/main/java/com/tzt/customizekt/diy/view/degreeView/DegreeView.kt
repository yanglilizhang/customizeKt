...
import com.tzt.customizekt.DisplayHelper
...
//        mProgressText = (mProgressText * 1.0 / (mMaxProgress - mMinProgress) * 100).toInt()
        mProgressText =
            ((mSlidePoint.x - mSlideMaxRadius * 2f) / mLimit + mMinProgress + 0.5f).toInt()
        Log.e(
            "DegressView",
            "progress[${((mSlidePoint.x - mSlideMaxRadius * 2f) / mLimit + mMinProgress + 0.5f).toInt()}],[${mProgressText}],min[$mMinProgress],max[$mMaxProgress]"
        )
...
//                mSlideRadius * 2f,
    ...
    private fun drawSlide(canvas: Canvas) {
        ...
        canvas.drawPoint(mSlidePoint.x, mSlidePoint.y, mTextPaint)
    }
...
//            mSlidePoint.y + mSlideRadius + mTextHeight / 2 + maxDistance + 10f,
            mSlidePoint.y + mSlideMaxRadius + 10f + mTextHeight / 2 + maxDistance,
            mTextPaint
        )
    }
